...
const logInUser = asyncHandler(async (req, res) => {
  ...
  // send cookies
  // send response for succcessfull login
...
const updateUserAvtar = asyncHandler(async (req, res) => {
  ...
  if (!avtarLocalPath) {
    throw new ApiError(400, "Avtar is required to change");
  }
  const avtar = await uploadOnCloudinary(avtarLocalPath);
  if (!avtar.url) {
    throw new ApiError(400, "Avtar not uploaded on cloudinary successfully");
  }
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  if (!avtarLocalPath) {
    throw new ApiError(400, "Cover image is required to change");
  }
  const coverImage = await uploadOnCloudinary(coverImageLocalPath);
  if (!coverImage.url) {
    throw new ApiError(
      400,
      "CoverImage not uploaded on cloudinary successfully"
    );
  }
  const user = await User.findByIdAndUpdate(
    req.user._id,
